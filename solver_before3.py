...
class Solver(object):
    ...
    def save_recon(self, iters, index, XA, XB,
        ...
        logalpha, logalphaA, logalphaB = logalpha.cpu().detach().numpy(), logalphaA.cpu().detach().numpy(), logalphaB.cpu().detach().numpy()
    ...
    def save_synth_cross_modal(self, iters, howmany=3):
        ...
        for k in range(howmany):
            ...
            XB_synth_list.append(XB_synth.data)
            ZB = ZB.detach().cpu().numpy()
            ZB_random_list.append(ZB)
        ...
        for k in range(howmany):
            ...
            XA_synth_list.append(XA_synth.data)
            ZA = ZA.detach().cpu().numpy()
            ZA_random_list.append(ZA)


        ZS_infA = ZS_infA.detach().cpu().numpy()
        ZS_infB = ZS_infB.detach().cpu().numpy()
        ...
        for i in range(len(fixed_idxs)):
            ...
            plt.legend(['XA: amp %.2f, freq %.2f' % (self.sampled_amp_A[fixed_idxs[i]], self.sampled_freq[fixed_idxs[i]]),
                       ...
                       prop={'size': 13})
            plt.title("GT", fontsize=13)


            ## draw A2B
            for k in range(howmany):
                plt.subplot(3,3,4+k)
                xb_synth = XB_synth_list[k].detach().cpu().numpy()[i]
                plt.plot(x_range, xa)
                plt.plot(x_range, xb_synth)
                plt.axis([x_range[0], x_range[-1], y_min, y_max])

                plt.legend(['XA: amp %.2f, freq %.2f' % (self.sampled_amp_A[fixed_idxs[i]], self.sampled_freq[fixed_idxs[i]]),
                            'XB_Synth: ZB_rand %.2f, ZS_infA %.2f' % (ZB_random_list[k][i], ZS_infA[i])], loc='upper left',
                           prop={'size': 13})
                plt.title("XB_Synth", fontsize=13)

            ## draw B2A
            for k in range(howmany):
                plt.subplot(3,3,7+k)
                xa_synth = XA_synth_list[k].detach().cpu().numpy()[i]
                plt.plot(x_range, xa_synth)
                plt.plot(x_range, xb)
                plt.axis([x_range[0], x_range[-1], y_min, y_max])

                plt.legend(['XA_Synth: ZA_rand %.2f, ZS_infB %.2f' % (ZA_random_list[k][i], ZS_infB[i]),
                            'XB: amp %.2f, freq %.2f' % (self.sampled_amp_B[fixed_idxs[i]], self.sampled_freq[fixed_idxs[i]])], loc='upper left',
                           prop={'size': 13})
                plt.title("XA_Synth", fontsize=13)
            ...
            plt_fname = os.path.join(
                self.output_dir_synth, str(iters),
                'plt_synth_cross_modal_%s.jpg' % i
            )
            save_image(
            tensor=plots.permute(0,3,1,2), filename=plt_fname, nrow=1, pad_value=1
            )
            plt.close(fig)
