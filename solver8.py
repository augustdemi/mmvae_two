...
class Solver(object):
    ...
    def save_synth_cross_modal(self, iters, howmany=3):
        ...
        for k in range(howmany):
            ...
            merged = torch.cat([merged, XB_synth[:,:,2:30, 2:30]], dim=0)
        ...
        merged = torch.cat([fixed_XB[:,:,2:30, 2:30]], dim=0)
        for k in range(howmany):
    ...
    def save_traverse(self, iters, logalpha, logalphaA, logalphaB, z_A, z_B, z_S, loc=-1):
        ...
        decoderA = self.decoderA
        decoderB = self.decoderB
        interpolationA = torch.tensor(np.linspace(min(z_A), max(z_A), 30))
        interpolationB = torch.tensor(np.linspace(min(z_B), max(z_B), 30))
        interpolationS = torch.tensor(np.linspace(min(z_S), max(z_S), 30))

        print('------------ traverse interpolation ------------')
        print('interpolationA: ', min(z_A), max(z_A) )
        print('interpolationB: ', min(z_B), max(z_B) )
        print('interpolationS: ', min(z_S), max(z_S) )
        ...
        ####
            ...
            for i, idx in enumerate(fixed_idxs):
                IMG['fixed' + str(i)] = \
                    torch.cat([fixed_XA_3ch[i], fixed_XB[i][:,:,2:30, 2:30]], dim=2)
            each_XA = random_XA.clone().squeeze()
            IMG['random'] = torch.cat([torch.stack([each_XA, each_XA, each_XA]).unsqueeze(0), random_XB[:,:,2:30, 2:30]], dim=2)
        ...
        for key in Z:
            ...
            for row in range(self.zA_dim):
                ...
                for val in interpolationA:
                    ...
                    sample = torch.cat([torch.stack(sampleA_3ch), sampleB[:,:,2:30,2:30]], dim=2)
                    gifs.append(sample)
        ...
        gifs = gifs.view(
            ...
            len(interpolationA), self.nc, 2 * 28, 28
        ).transpose(1, 2)
